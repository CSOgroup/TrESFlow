...
`*_MarkedDup.bam` is generated by nf-core `gatk4/markduplicates` with TrESFlow's barcode-aware duplicate settings and then normalized back to the historical TrESFlow filename contract. `SPLIT_DUPLICATES_DNA` writes only `*_NoDup.bam` (records without flag `0x400`); it does not create an unused duplicate-only BAM. `*_NoDup.bam` remains the input to the mapped-read precheck, samtools QC, and downstream DNA coverage. `*_NoDup.bw` is generated via nf-core `deeptools/bamcoverage` after the pipeline confirms that the NoDup BAM has mapped reads.
