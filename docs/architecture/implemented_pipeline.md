...
- Every read role is normalized to an ordered path list. Equal positions form synchronized technical read sets from one logical library. Collision-safe indexed staging plus manifest files carries those paths to Python/Codon without comma-joining or raw-FASTQ concatenation; the first tagger emits the existing single combined output pair and aggregate metrics.
- Python mock and production Codon taggers open each manifest source according to its compression, stream sources in order, and require complete records, per-set equal exhaustion, and normalized QNAME agreement for every record. An internal record-count sidecar preserves technical-set boundaries while the combined tagged pair passes through later taggers, so compensating length errors cannot cross a chunk boundary. The sidecar is not published. Legacy direct single-FASTQ Codon calls remain valid.
- `rna_sb_group_map.tsv`, `dna_sb_group_map.tsv`, `dna_mo_map.tsv`, DNA modality whitelist files, and `input_fastq_provenance.tsv` are derived internal artifacts, not user inputs.
