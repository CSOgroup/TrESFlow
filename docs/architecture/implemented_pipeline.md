...
- nf-core `gatk4/markduplicates` replaces the previous local GATK invocation and keeps duplicate grouping cell-aware with `--BARCODE_TAG CB`. DNA `RG` values are lane-qualified (`<CB>_L<lane>`) for AVITI physical-coordinate separation, while every lane-specific header record uses the same logical `LB`, preserving genomic duplicate grouping across lanes. TrESFlow supplies the AVITI QNAME regex and an empirical, configurable optical distance (default 10 AVITI coordinate units). `--REMOVE_DUPLICATES false`, index creation, and the historical TrESFlow output names remain unchanged through the normalization adapter.
